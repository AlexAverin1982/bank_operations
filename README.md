# Домашняя работа 13.2 Поиск и фильтрация с помощью регулярных выражений, использование счетчиков Counter. Объединение всех модулей в одно приложение
## Цель домашней работы
Научиться использовать инструменты для поиска данных с использованием регулярных выражений.
Научиться использовать инструменты из модуля Collections для получения статистических данных
Объединить наработки всех домашних работ в одно рабочее приложение.

## Реализация инструментария re
### Функция `find_matches_in_description(operations: list[dict], pattern: str, case_sensitive: bool = False)`
  + #### принимает на вход 
    + данные в виде списка словарей; 
    + строковое регулярное выражение для поиска в данных;
    + флаг, включающий чувствительность к регистру (по умолчанию выключено)
  + #### возвращает
     список словарей, каждый из которых содержит данные по отдельной транзакции

## Реализация статистического инструментария 
### Функция `count_categories(operations: list[dict], categories: list) -> dict:`
  + #### принимает на вход 
    + данные в виде списка словарей; 
    + список строк с категориями, данные по которым нужно подсчитать;
  + #### возвращает
     словарь, содержащий количество записей по каждой указанной категории
...
## Инструкции по использованию общего приложения:
Вызов функции `main()` из модуля `main.py` предоставляет демонстративный набор функций из всех модулей с реализацией пользовательского интерфейса в виде набора меню в консоли.  
